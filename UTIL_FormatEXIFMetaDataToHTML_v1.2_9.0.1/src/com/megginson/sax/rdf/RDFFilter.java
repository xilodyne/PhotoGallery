...
// $Id: RDFFilter.java,v 1.3 2001-06-05 16:36:28 bwm Exp $
 ...
 *
 * <p>The system property "http://megginson.com/sax/rdf/damlns" defines the
 * namespace that will be used for DAML resources and properties.</p>
 *
...

	...
	locator = null;
	idTable = new Hashtable<String, Boolean>();
...

    ...
    {
    	//austin
    	//System.out.println("filter ln329 startElement, uri: " + uri );
	...
	case IN_UNKNOWN:
	    // NO-OP;
	    break;
                                // First Element in a DAML collection
        case START_DAML_COLLECTION:
            currentState.state = IN_DAML_COLLECTION_ELEMENT;
            currentState.previousState.state = IN_DAML_COLLECTION;
            startResource(uri, localName, atts);
            break;
                                // subsequent elements in a DAML collection
        case IN_DAML_COLLECTION:
            currentState.state = IN_DAML_COLLECTION_ELEMENT;
            continueDAMLCollection();
            startResource(uri, localName, atts);
            break;            
                                // nest properties of a DAML collection element
        case IN_DAML_COLLECTION_ELEMENT:
	    currentState.state = IN_PROPERTY;
	    startProperty(uri, localName, atts);
	    break;
	...
	default:
	    throw new RuntimeException("StartElement:Unexpected state " + 
                                        currentState.state);
	}
    }
...

    ...
    {
    	//austin
    	//System.out.println("ln759 startRe, uri: " + uri + ", name: " + name +", att:" + atts);
...
//bwm this is incorrect.  Need to output a type statement when the namespace
//bwm is not RDFNS.  Move this block out of the enclosing condition.
//bwm	    if (!"Description".equals(name)) {
//bwm	    reportStatement(currentState.subject, RDF_TYPE, uri + name,
//bwm			   RDFHandler.SUBJECT_URI, false, null, null);
//bwm	    }
	}
        if (! "Description".equals(name)) {                         //bwm ins
            reportStatement(currentState.subject, RDF_TYPE,         //bwm ins
                            uri + name, currentState.subjectType,   //bwm ins
                            false, null, null);                     //bwm ins
        }                                                           //bwm ins
	...
	//austin
	//System.out.println("ln935, id: " + id + ", about: " + about +
	//		", each: " + aboutEach +", prefix: " + aboutEachPrefix + 
	//		", bagID: " + bagId);
	...
	throws SAXException
    {
    	...
    	//austin
    	//System.out.println("ln990 startProp, uri: " + uri);
	...
	for (int i = 0; i < atts.getLength(); i++) {
		...
		if ("resource".equals(aname)) {
			...
			parseType = "Resource";
                    } else if ("daml:collection".equals(value)) {
	...
	if (parseType != null) {
		...
		if (rdfHandler != null) {
            ...
            } else if (parseType == "daml:collection") {
                // bwm ins
                // treat like a parseType resource and create an anon resource
                // to be the head of the list
                String newSubject = genID();;
                reportStatement (currentState.subject, currentState.predicate,
		                 newSubject, currentState.subjectType, false,
				 null, null);
                reportStatement(newSubject, RDF_TYPE, DAML_LIST,
                                 RDFHandler.SUBJECT_GENERATED, false, 
                                 null, null);  // set type of anon node
                pushState();
                currentState.state = START_DAML_COLLECTION;
...
//bwm There was a problem here when attributes in a property element
//bwm implied that there was an anon resource inserted as the object
//bwm of the current property.  It is necessay to change
//bwm the subjectType while reportPropAtts is being called so
//bwm that the correct subjectType is reported to client.
                    
            int oldSubjectType = currentState.subjectType;          //bwm ins

	    if (resource == null) {
		resource = genID();
                currentState.subjectType = 
                                      RDFHandler.SUBJECT_GENERATED; //bwm ins
	    }
	    if (bagId != null) {
	    reportStatement(bagId, RDF_TYPE, RDF_BAG,
			   RDFHandler.SUBJECT_URI, false, null, null);
	    ...
	    currentState.subject = oldSubject;
            currentState.subjectType = oldSubjectType;              //bwm ins
	}
    }
	...
	private void startDAMLCollection() throws SAXException {	
        String subject = currentState.subject;
        int subjectType = currentState.subjectType;
        pushState();
        currentState.subject = subject;
        currentState.subjectType = subjectType;
    }
    
    private void continueDAMLCollection() throws SAXException {
        String newSubject = genID();
        reportStatement(currentState.previousState.subject, DAML_REST,
                        newSubject, currentState.previousState.subjectType,
                        false,null, null);
        reportStatement(newSubject, RDF_TYPE, DAML_LIST,
                        RDFHandler.SUBJECT_GENERATED, false, null, null);
        currentState.previousState.subject = newSubject;
    }
    
    private void endDAMLCollectionElement() throws SAXException {
        reportStatement (currentState.previousState.subject, DAML_FIRST,
		         currentState.subject, RDFHandler.SUBJECT_GENERATED,
                         false, null, null);
    }
    
    private void endDAMLCollection() throws SAXException {
        reportStatement (currentState.subject, DAML_REST,
		         DAML_NIL, RDFHandler.SUBJECT_GENERATED,
                         false, null, null);
        popState();
    }
    ////////////////////////////////////////////////////////////////////
    ...
    {
        // check for containerMembershipProperty rdf:_nnn       //bwm ins
                                                                //bwm ins
        if (name.charAt(0)=='_') {                              //bwm ins
            // could be - check the rest are decimal digits     //bwm ins
            for (int i=1; i<name.length(); i++) {               //bwm ins
                char ch = name.charAt(i);                       //bwm ins
                if (ch < '0' || ch > '9') {                     //bwm ins
                    return false;                               //bwm ins
                }                                               //bwm ins
            }                                                   //bwm ins
            return true;                                        //bwm ins
        }                                                       //bwm ins
...

    ...
    private final static String DAMLNSURIPROP =
                                 "http://megginson.com/sax/rdf/damlns";    
    private final static String DAML_DEFAULT = 
                                 "http://www.daml.org/2001/03/daml+oil.daml#";    
    private final static String DAML = System.getProperty(DAMLNSURIPROP,
                                                     DAML_DEFAULT).intern();
    private final static String DAML_LIST  = (DAML + "List").intern();
    private final static String DAML_FIRST = (DAML + "first").intern();
    private final static String DAML_REST  = (DAML + "rest").intern();
    private final static String DAML_NIL   = (DAML + "nil").intern();
    ...
    private final static int START_DAML_COLLECTION = 12;
    private final static int IN_DAML_COLLECTION = 14;
    private final static int IN_DAML_COLLECTION_ELEMENT = 15;
...

    ...
    private Hashtable<String, Boolean> idTable;
